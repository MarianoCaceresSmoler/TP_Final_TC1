...
\begin{block}{Carga y visualización}
\scriptsize
\begin{itemize}
    \item Carga de CSV de osciloscopio, LTspice y Bode.
    \item Soporta cantidad variable de canales.
    \item Drag \& drop de archivos.
    \item Modo exclusivo: tiempo o Bode.
    \item Superposición de varios CSV compatibles.
    \item Título, nombres de ejes y unidades configurables.
    \item Grilla configurable, escalas logarítmicas y exportación a PDF.
\end{itemize}
\end{block}

\begin{block}{Edición por canal}
\scriptsize
\begin{itemize}
    \item Activar/desactivar canales.
    \item Cambio de color.
    \item Escala vertical y offset vertical.
    \item Desplazamiento temporal con mouse, flechas o valor escrito.
...
\scriptsize
\begin{itemize}
    \item Cursores múltiples por canal.
    \item $Y$ automática al elegir $X$.
    \item Líneas verticales y horizontales.
    \item Comparación entre cursores: $\Delta X$ y $\Delta Y$.
    \item Movimiento de cursores con mouse, teclado o coordenada escrita.
    \item Marcado automático de máximos y mínimos.
\end{itemize}
\end{block}

\begin{block}{Modos especiales}
\scriptsize
\begin{itemize}
    \item Modo Bode: magnitud en dB y fase en grados, con frecuencia logarítmica.
    \item Modo XY/Lissajous para comparar dos señales temporales.
    \item Rechazo de archivos inválidos sin cerrar la aplicación.
\end{itemize}
\end{block}
